...
class RewardNormalizer:
    ...
    def __call__(self, reward, first):
        rets = backward_discounted_sum(
            prev_ret=self.ret, reward=reward.cpu(),
            first=first.cpu(), gamma=self.gamma)
        self.ret = rets[:, -1]
...
def backward_discounted_sum(prev_ret, reward, first, gamma,):
    first = first.to(dtype=torch.float32)
    assert first.dim() == 2
    _nenv, nstep = reward.shape
    ret = torch.zeros_like(reward)
    for t in range(nstep):
        prev_ret = ret[:, t] = reward[:, t] + (1 - first[:, t]) * gamma * prev_ret
    return ret
